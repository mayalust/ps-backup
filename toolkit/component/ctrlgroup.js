...
define([], function () {
  return function (data) {
    ...
    function runExpressionSuccess(expression) {
      ...
      // 创建了多个下拉框
      var createMultipleSelect = function (fmt, ctrlGroup) {
        var format = fmt.format || {
          id: "id",
          label: "label"
        };
      ...
      var createInputableSelect = function (fmt, ctrlGroup) {
        ...
        addBtn.on("click", function () {
          ...
          obj[format.label] = inputValue;
          //防止添加重复类型
          let rs = options.find(v => v.label === obj.label);
          if (!rs) {
            options.push(obj);
          } else {
            growl.info("该备件类型已存在");
      ...
      var createDateTimePicker = function (fmt, ctrlGroup) {
        ...
        setTimeout(function () {
          $$.loadExternalJs(['laydate'], function (laydate) {
            ...
            if (typeof value === "object" && value.CLASS === "DateHandler") {
              if (dateType == "date")
                value.trimmToDate();
              ...
              value: (fmt.value == '' || fmt.value == undefined) ? '' : element.getDateHandler(fmt.value[0]).getDateString("yy-MM-dd") + " - " + element.getDateHandler(fmt.value[1]).getDateString("yy-MM-dd"),
                ...
                if (changeFn) {
                  changeFn({
                    ...
                    current: fmt,
                    value: {
                      start: dateArray[0] == '' ? null : formatDatebyMomentToUTC(dateArray[0]), //格林尼治时间格式
                      end: dateArray[0] == '' ? null : formatDatebyMomentToUTC(dateArray[1]), //格林尼治时间格式
