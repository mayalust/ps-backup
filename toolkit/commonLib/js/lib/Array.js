(function(window){
  ...
  Object.prototype.$delete = function() {
    var target = this, path, arr = arguments[0].split("/");
    while(path = arr.pop()){
      if(arr.length)
        target = target[path];
      else
  ...
  Array.prototype.$sum = function(callcack,ispic){
    var sum = 0;
    for(var i in this){
      sum += callcack(this[i]);
    }
  ...
  function clone(/*Object*/) {
    ...
    function traverse(target, children, parents) {
      parents = parents || [];
      if(parents.indexOf(children) !== -1){
        throw new Error("circular JSON cannot be cloned!");
        return;
      }
      var inx = 0;
      for(var i in children) {
        if(typeof children[i] == 'object' && children[i] != null && !(children[i] instanceof Date)) {
          ...
          if(children.hasOwnProperty(i)) {
            ...
            traverse(target[i], children[i], parents.concat([children]));
          }
