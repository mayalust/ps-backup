define(['../services/services.js'], function (services) {
  'use strict';
  services.factory('resourceUIService', ['serviceProxy', '$q',
    function (serviceProxy, q) {
      var deviceLoader = createPromise(),
        modelLoader = createPromise(),
        unitLoader = createPromise(),
        domainsLoader = createPromise(),
        deviceObjLoader = createPromise(),
        incloseDeviceLoader = createPromise(),
        service = {
          code: 0,
          nodesDic: {},
          kpisDic: {},
        ...
        resourceUIName = "resourceUIService";
      ...
      service.domainsLoader = function (d) {
        if (typeof d === "object") {
          domainsLoader.resolve(d)
        } else {
          return domainsLoader.promise
        }
      }

      service.deviceObjLoader = function (d) {
        if (typeof d === "object") {
          deviceObjLoader.resolve(d)
        } else {
          return deviceObjLoader.promise
        }
      }

      service.unitLoader = function (d) {
        if (typeof d === "object") {
          unitLoader.resolve(d)
        } else {
          return unitLoader.promise
        }
      }

      service.deviceLoader = function (d) {
        if (typeof d === "object") {
          deviceLoader.resolve(d)
        } else {
          return deviceLoader.promise
        }
      }

      service.modelLoader = function (d) {
        if (typeof d === "object") {
          modelLoader.resolve(d)
        } else {
          return modelLoader.promise;
        }
      }

      service.incloseDeviceLoader = function (d) {
        if (typeof d === "object") {
          modelLoader.resolve(d)
        } else {
          return modelLoader.promise;
       ...
       * @param callBack
       */
      service.updateDeviceValues = function (condition, keyValueList, callBack) {
        serviceProxy.get(resourceUIName, "updateDeviceValues", [condition, keyValueList], callBack);
      };
      /**
       * 查询用户企业域下的接入用户
       */
       ...
       * 通过模型ID获取KPI定义列表
       */
      service.getDataItemsByModelId = function (id, callBack) {
        serviceProxy.get(resourceUIName, "getDataItemsByModelId", [id], callBack);
      };
      /**
       * 查询接入协议和解析协议
       */
       ...
       * 型号id
       */
      service.findCollectionTaskDefinitionByModelId = function (id, callBack) {
        serviceProxy.get(resourceUIName, "findCollectionTaskDefinitionByModelId", id, callBack);
      };
      /**
       * 获取默认属性定义列表
       */
       ...
       * 获取所有资源
       */
      service.getResources = function (callBack, extendstr) {
        serviceProxy.get(resourceUIName, "getResources", null, callBack, null, extendstr);
      };

      service.getResourceByModelId = function (id, callBack, extendstr) {
        serviceProxy.get(resourceUIName, "getResourceByModelId", id, callBack, null, extendstr);
      };
      ...
      //条件查询域
      service.getDomainsByFilters = function (param, callback) {
        serviceProxy.get(resourceUIName, 'getDomainsByFilters', param, callback);
      };
      //查企业
       ...
       * @param callback
       */
      service.getDevicesByUserId = function (userId, params, callback) {
        serviceProxy.get(resourceUIName, 'getDevicesByUserId?includeFields=label,id,domains', [userId, params], callback);
      };
      ...
      service.getDevicesByCondition({}, function (returnObj) {
        if (returnObj.code == 0) {
          service.deviceLoader(returnObj.data);
        }
      }, "includeFields=label,customerId,externalDevId,id,values.images,domains,modelId");

      /**
      var params = {},
        currentRoleID = userLoginUIService.user.currentRoleID || 0,
        roleFunctionCodeMap = userLoginUIService.user.roleFunctionCodeMap || {},
      ...
      service.getDevicesByCondition(params, function (returnObj) {
        if (returnObj.code == 0) {
          service.incloseDeviceLoader( returnObj.data );
        }
      }, "includeFields=id");**/
      return service;
    }
