define(['controllers/controllers', 'bootstrap-dialog', 'bootstrap-multiselect'], function (controllers, BootstrapDialog, bootstrapMultiselect) {
  ...
  controllers.initController('workFlowCtrl', ['$scope', '$q', 'ticketCategoryService', '$location', 'workflowService', '$timeout', 'userRoleUIService',
    'kpiDataService', 'resourceUIService', 'growl', 'userLoginUIService', 'processService', 'userEnterpriseService', 'Info', '$route', 'commonMethodService',
    function ($scope, q, ticketCategoryService, $location, workflowService, $timeout, userRoleUIService, kpiDataService, resourceUIService, growl, userLoginUIService, processService, userEnterpriseService, Info, $route, cmd) {
      $scope.workOrderType = "";
      $scope.flows = "";
      var uuid = Math.uuid();
      var cmdins = new cmd();
      ...
      $scope.tabStatus = function (type, handerConfsData) {
        if (handerConfsData) {
          ...
          if (type == "role") {
             ...
             * 韩星
             */
            $scope.$broadcast("handerConfsData",handerConfsData[$scope.flowList.taskId]);
            // $scope.roleIdUser();
          } else if (type == "user") {
