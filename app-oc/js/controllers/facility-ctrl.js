define(['controllers/controllers', 'bootstrap-dialog'], function (controllers, BootstrapDialog) {
  'use strict';
  controllers.initController('FacilityCtrl', ['$scope', '$q', 'FileUploader', '$controller', '$routeParams', 'growl', 'userUIService', 'userEnterpriseService', 'kpiDataService', 'resourceUIService', 'Info',
    'viewFlexService', 'userDomainService', 'userLoginUIService', 'customerUIService', 'projectUIService', 'ngDialog', 'deviceAccessUIService', 'SwSocket',
    function ($scope, q, FileUploader, $controller, $routeParams, growl, userUIService, userEnterpriseService, kpiDataService, resourceUIService, Info,
              viewFlexService, userDomainService, userLoginUIService, customerUIService, projectUIService, ngDialog, deviceAccessUIService, SwSocket) {
      /**上传图片开始*/
      ...
      $scope.showMeasurePoint = function () {
        ...
        for (var i in propsList) {
          $scope.specialtyProps.push({value: propsList[i].valueCode, label: propsList[i].label});
        }
        ...
        var checkoutSelectVerify = function () {
          if (!$scope.pointType) {
      ...
      $scope.dialogQueryModelChange = function () {
        if (resourceUIService.rootModelsDic[$scope.queryDitem.modelId]) {
          var queryAttrList = []
          resourceUIService.rootModelsDic[$scope.queryDitem.modelId].attrs.forEach(function (attr) {
            if (attr.values.sensitive != '1' || $scope.menuitems['A08_S02']) {
          ...
          $scope.dialogQueryAttrList = queryAttrList;
        } else {
          var attrs = [];
          for (var key in $scope.queryAttrObj) {
            attrs.push($scope.queryAttrObj[key])
          }
          $scope.dialogQueryAttrList = attrs;
        }
      }
      $scope.$on('SET_ATTR_BATCH', function () {
        if ($scope.selectedDevices.length < 1) {
          growl.warning('还没有选择设备');
          return;
        }
        $scope.cityChang = {};
        $scope.isAllSelect = false;
        $scope.dialogQueryKeyValue = [];
        $scope.dialogQueryModelChange();
        ngDialog.open({
          template: 'partials/set_attr_batch.html',
          scope: $scope,
          className: 'ngdialog-theme-plain'
        });
      });

      $scope.$on('SET_ALL_ATTR_BATCH', function () {
      ...
      $scope.dialogSelQueryAttr = {
        "attr": ""
      };
      $scope.selectedHandler = function (rows) {
        $scope.selectedDevices = rows;
      };

      $scope.attrDialogChang = function () {
        for (var b in $scope.dialogQueryAttrList) {
          if ($scope.dialogQueryAttrList[b].uid == $scope.dialogSelQueryAttr.attr) {
            if ($scope.dialogQueryAttrList[b].dataType == 'standardAddress') {
              $scope.dialogQueryAttrList[b].province = '';
              $scope.dialogQueryAttrList[b].city = '';
              $scope.dialogQueryAttrList[b].district = '';
            }
            $scope.dialogQueryKeyValue.push($scope.dialogQueryAttrList[b]);
            $scope.dialogQueryAttrList.splice(b, 1);
            break;
          }
        }
      };

      $scope.removeDialogAttr = function (id) {
        for (var b in $scope.dialogQueryKeyValue) {
          if ($scope.dialogQueryKeyValue[b].id == id) {
            $scope.dialogQueryAttrList.push($scope.dialogQueryKeyValue[b]);
            $scope.dialogQueryKeyValue[b].endTime = '';
            $scope.dialogQueryKeyValue[b].value = '';
            $scope.dialogQueryKeyValue[b].startdate = '';
            $scope.dialogQueryKeyValue[b].endNumberic = '';
            $scope.dialogQueryKeyValue[b].startNumberic = '';
            $scope.dialogQueryKeyValue.splice(b, 1);
            break;
          }
        }
      };
      ...
      $scope.setAttr = function () {
        ...
        if ($scope.isAllSelect) {
          if ($scope.queryDitem.modelId) {
            queryDitemList["modelId"] = $scope.queryDitem.modelId;
          }
          if ($scope.queryDitem.domainPath) {
            queryDitemList["domains"] = $scope.queryDitem.domainPath;
          }
          if ($scope.queryDitem.customerId) {
            queryDitemList["customerId"] = $scope.queryDitem.customerId;
          }
          if ($scope.queryDitem.projectId) {
            queryDitemList["projectId"] = $scope.queryDitem.projectId;
          }
          if ($scope.queryDitem.sn) {
            queryDitemList["orCondition"] = $scope.queryDitem.sn;
            queryDitemList["conditionField"] = ["sn", "label"];
          }
          if ($scope.seniorQuery) {
            var objQuery = {};
            for (var o in $scope.queryKeyValue) {
            ...
            queryDitemList["values"] = objQuery;
          }
        } else {
          queryDitemList.resourceIds = $scope.selectedDevices.map(function (item) {
            return item.id;
          });
        }

        var queryKeyValueObj = {};
        $scope.dialogQueryKeyValue.forEach(function (item) {
          queryKeyValueObj[item.name] = item.value || '';
        });
        resourceUIService.updateDeviceValues(queryDitemList, queryKeyValueObj, function (ret) {
          if (ret.code == 0) {
            growl.success('设置成功');
            $scope.closeDialog();
          }
