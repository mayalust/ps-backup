  ...
  class filetree {
    ...
    stat(name) {
      ...
      typeof name !== "undefined" ?
        path.push(pathResolver.join("./", name)) :
        null;
    ...
    write(name, content) {
      ...
      return typeof content === "undefined" ?
        ...
        write(path, content).then(d => getState(path));
    }
    read(name) {
    ...
    mkdir(name) {
      ...
      return mkdir(path).then(d => getState(path));
    }
    find(callback) {
      if (typeof callback !== "function") {
        throw new Error(
          "query condition is not a valid function but a " + typeof callback
        );
      }
      ...
      fs.readdir(path, (err, d) => {
        if (!err) {
          return Promise.all(
              d.map(n => {
                return getState(
                  pathLib.resolve(path, pathResolver.join("./", n))
                );
              })
            )
            .then(d => {
        ...
        if (!err) {
          resolve(
            new filetree({
              ...
              isDir: d.isDirectory(),
              modifytime: d.mtime,
              modifytimeStr: dateparser(d.mtime).getDateString(
                "yyyy-MM-dd,hh:mm:ss"
              ),
              data: d,
              ext: getExt(path)
