          ...
          target.$getKpisByModelId(modelId, function(subData) {
            var getValueList = function(valueList) {
              ...
              function callback(returnData) {
                ...
                var render = function() {
                  ...
                  tableIns = target.render(ctrlGroups);
                };
                render();
              };
            }
            target.webSocket(ci, kpi, function(data) {
              var nodeId = data.nodeId,
                ...
                value = data.value,
                time = data.arisingTime;
              refresh(nodeId, kpiCode, value, time);
            }, "instance_parameter");

            function refresh(nodeId, kpiCode, value, time) {
              if (!tableIns) {
                return;
              }
              var inx = options.findIndex(function(d) {
                  return d.dataItemId == kpiCode;
                }),
                unit = units[kpiCode] || "",
                trs = tableIns.find("tr"),
                tr = trs[inx + 1],
                tds = tr.getElementsByTagName("td"),
                spa1 = tds[1].getElementsByTagName("span")[2],
                spa2 = tds[2].getElementsByTagName("span")[2];
              spa1.innerText = value + (typeof unit === "object" ? unit.unitName : unit);
              spa2.innerText = target.dateHandler(time).getDateString("yy/MM/dd hh:mm:ss");
            }
